...
{
    QStringList enabledPatterns;
    enabledPatterns.reserve(static_cast<int>(patternList.size()));
    for (const FilePattern & pattern : patternList) {
        if (pattern.enabled)
            enabledPatterns.push_back(pattern.pattern);
    }
